...
if mode=="form":
    if not form_id or "forms" not in meta or form_id not in meta["forms"]:
        ...
        if submitted:
            row={ "FormID":form_id, "FormName":info["form_name"], "UserSession":session_id, 
...
# ----------------------------
    ...
    if member_file and form_file:
        try:
            ...
            edited_df=st.data_editor(st.session_state.current_form_df, use_container_width=True, num_rows="dynamic", key="form_editor")
            ...
            if st.button("🚀 Create Form & Send Emails"):
                if not base_url:
                    ...
                    forms[form_id_new]={ "form_name":form_name, "columns":list(st.session_state.current_form_df.columns),
                                         "dropdowns":dropdowns, "created_at":datetime.now().isoformat() }
                    meta["forms"]=forms
                    save_meta(meta)
    ...
    if responses.empty:
        ...
        if not responses_display.empty:
            ...
            to_download.seek(0)
            st.download_button(label="📥 Download Responses", data=to_download, file_name="responses.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
